...
class MultinomialCost(BaseCost):
    ...
    NOTE (#44): must subclass ``BaseCost`` — ``rpt.Pelt(custom_cost=...)``
    SILENTLY falls back to CostL2 for non-BaseCost objects, so experiment
    #43's M4 actually segmented with an L2 cost on raw counts. The 'l2'
