...
from src.run_scale import (BAND_LEVEL, FIXED_PARAMS, N_TOTAL, ROWS,
                           ...
                           canonical_tasks, gate_a_repro, gate_b_e2e,
                           noise_floors, parse_shard, prepare_question,
                           question_meta, rec_path, spot_rows_by_track)
...
def run_fan(args):
    ...
    tasks = canonical_tasks(tracks, rows, s_grid=ABL_S_GRID,
                            strides=ABL_STRIDES)
    if args.limit:
    ...
    meta = {"s_grid": ABL_S_GRID, "strides": ABL_STRIDES,
            ...
            "tracks": tracks, "rows": rows, "shard": args.shard or None,
            "n_tasks": len(tasks),
...
def main():
    ...
    ap.add_argument("--meta-only", action="store_true",
                    help="write questions meta and exit")
